...
for tl_0_below_threshold, tl_0_above_threshold in list_t0s:
    ...
    print(
        "Green light durations on each route are (%s,%s) below threshold, (%s,%s) above threshold"
        % (
    ...
    tl_list = [
        (tl_0_below_threshold, tl_1_below_threshold, tl_y, nb_agents)
        if nb_1 < threshold
        else (tl_0_above_threshold, tl_1_above_threshold, tl_y, nb_agents)
...
for id in range(1024):
    if nash_equilibrium(id_to_strategy(id)):
        ...
        if strong_nash_equilibrium(id_to_strategy(id)):
            print("It is a strong nash equilibrium:", id_to_strategy(id))
print("Done")
...
tl_0_below_threshold = 24   # how long the green light lasts on route 0 before traffic lights adapt
tl_0_above_threshold = 16   # how long the green light lasts on route 0 after traffic lights adapt
tl_1_below_threshold = cycle_length - 2*tl_y  - tl_0_below_threshold
tl_1_above_threshold = cycle_length - 2*tl_y  - tl_0_above_threshold
...
output_file = "coalition_deviation_report_2.txt"
