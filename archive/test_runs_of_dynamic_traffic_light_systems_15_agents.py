...
def custom_df(tl_list, custom_filename = "reward_df_custom.csv"):
    ...
    for id in range(1024):
        ...
        #print("tls are: ", tl_0,tl_1,tl_y,nb_agents, "\n\n")
        try:
...
tl_0_above_threshold = 8    # how long the green light lasts on route 0 after traffic lights adapt
...
output_file = f"coalition_deviation_results_{tl_0_below_threshold}_{tl_0_above_threshold}.txt"
...
with open(output_file, "w", encoding="utf-8") as f:
    with redirect_stdout(f):
        ...
        print("#######################")
        print("Initial situation: everyone on route 0.")
        ...
        print(
            ...
            % (nash_equilibrium(s_0), strong_nash_equilibrium(s_0, verbose=False))
        )
        ...
        def explore_deviations(club, nb_tabs=1):
            ...
            print(
                nb_tabs * "\t"
                + "List of coalitions that could deviate simultaneously: %s" % new_c_dev
            )
            ...
            print(
                nb_tabs * "\t"
                + "Is x^0 = (0,0,...,0) a Nash equilibrium? :",
                nash_equilibrium(id_to_strategy(0))
            )

            # checking club internal stability
            ...
            if not internal_stability:
                print(
                    nb_tabs * "\t"
                    + "Club is not internally stable "
                    + "(at least one member of the club wants to deviate)"
                )

            for i in club:
                print(
                    nb_tabs * "\t"
                    + "%s gains %s from being in the club" % (i, rew[i] - rew_0[i])
                )
