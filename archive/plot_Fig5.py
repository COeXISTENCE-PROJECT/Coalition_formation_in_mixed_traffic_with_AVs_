...
def compute_stability_points(nb_agents, threshold=2, tl_y=5, cycle_length=50):
    ...
    for tl_0_below_threshold in range(15, 27):
        for tl_0_above_threshold in range(1, 26):
            ...
            print(
                "nb_agents = %s | Green light durations: "
                "(%s,%s) below threshold, (%s,%s) above threshold"
                % (
                    nb_agents,
            ...
            if nash_equilibrium(s0):
                if strong_nash_equilibrium(s0, verbose=False):
...
def plot_stability_panel(
    ...
    ax.set_title(title, **tnrfont, size=28)
